...
class Cgp(Cgp_algorithm):
    ...
    def __convertData(self):
        ...
        for i in xrange( self.trainingVectors):
    ...
    def run(self, generation, population, mutation):
        ...
        offset = 0
        for j in xrange( self.trainingVectors):
    ...
    def initCompile(self, population):
        arr = []
        for i in xrange( self.graphInputCnt + self.cols * self.rows):
            arr.append(i)
        self.mask_idx = i + 1
    ...
    def evaluatePop(self):
        ...
        for i in xrange( self.popsize):
            ...
            for k in xrange( self.graphOutputCnt):
                # Zero count
                # Int -> string -> zero_cnt
