...
include "cgp_circuit_sad.pyx"
include "cgp_functions.pyx"

DEF CIRCUIT = 1
DEF EQUATION = 0 # not implemented
...
data(input, output)
    ...
    def runSHD(self, generation, population, mutation, runs=None, acc=None):
        """Runs CGP algorithm.
...
run(generation, population, mutation, runs=None)
    ...
    @param runs       Number of runs of CGP.
        ...
        for i in range(runs):
            ...
            # Save results.
            if type_ == CIRCUIT:
                cgp_cir.setMaxGates(acc)
                cgp_cir.run(generation, population, mutation)
                result = (cgp_cir.result, cgp_cir.elapsed, cgp_cir.bestFitness, cgp_cir.bestResultGeneration, cgp_cir.resultUsedNodes)
                self.results.append(result)
                if self.bestFitness < cgp_cir.bestFitness:
                    self.maxFitness  = cgp_cir.maxFitness
                    self.bestFitness = cgp_cir.bestFitness
                    self.usedNodes   = cgp_cir.resultUsedNodes
    ...
    #  @param population Population of CGP. Includes parent.
    #  @param mutation Count of maximum gene mutations that can occurs in one chromosome mutation.
    def runSAD(self, generation, population, mutation, runs=None, acc=None):
        """Runs CGP algorithm.
...
run(generation, population, mutation, acc, runs=None)
    ...
    @param runs       Number of runs of CGP.
        ...
        if type_ == CIRCUIT:
            ...
            input = list(self.dataInput)
            output = list(self.dataOutput)
            cgp_cir = CgpCircuitSAD(self.cols, self.rows, self.levelBack, input, output, self.function)
            #Cgp.mask = len(bin(self.dataInput[0]))-2
            self.bestFitness = 0x7fffffff
        else:
...
#self.table = ["1_24", "2_71", "3_14", "add", "sub", "mul", "div", "neg", "if", "iflez", "lt", "lte", "gt", "gte", "eq", "and", "or", "not", "nand", "nor"]
    ...
    def getOutput(self, input):
        result = -1
        for bitShift in range(2**self.graphInputCnt):
            bin_number = []
            for a in self.dataInput:
                bin_number.append("1" if a & (1 << bitShift) else "0")
            number = int("0b" + "".join(bin_number), 2)
            if number == input:
                break
        bin_number = []
        for b in self.dataOutput:
            bin_number.append("1" if b & (1 << bitShift) else "0")
        return int("0b" + "".join(bin_number), 2)
        


    def errChrom(self, chrom, outputBuffer, reference):
        j = self.graphInputCnt
        idx = 0
        for i in range(self.graphInputCnt, self.rows * self.cols + self.graphInputCnt):
            a = int(outputBuffer[chrom[idx]]); idx += 1;
            b = int(outputBuffer[chrom[idx]]); idx += 1;
            f = chrom[idx]; idx += 1;

            if   f == 0: outputBuffer[j] = a
            elif f == 1: outputBuffer[j] = a & b
            elif f == 2: outputBuffer[j] = a | b
            elif f == 3: outputBuffer[j] = a ^ b
            elif f == 4: outputBuffer[j] = ~a 
            elif f == 5: outputBuffer[j] = ~(a & b)
            elif f == 6: outputBuffer[j] = ~(a | b)
            elif f == 7: outputBuffer[j] = ~(a ^ b)
            j += 1
        bin_result = []
        for i in range(self.graphOutputCnt):
            bin_result.append("1" if int(outputBuffer[chrom[idx]]) & 1 else "0")
            idx += 1 
        result = int("0b" + "".join(bin_result), 2)
        return result - reference 

    def createErrorTable(self, chrom, col_bin, row_bin):
        if (row_bin + col_bin) != self.graphInputCnt:
            raise Exception("Incorrect data")
        result = []
        dataBuff = [0]*self.rows*self.cols
        for row in range(2**row_bin):
            result_row = []
            a = list(bin(row)[2:].zfill(row_bin))
            for col in range(2**col_bin):
                b = list(bin(col)[2:].zfill(col_bin))
                outputBuffer = a+b+dataBuff
                referenceInput = int("0b" + "".join(a+b), 2)
                error = self.errChrom(chrom, outputBuffer, self.getOutput(referenceInput))
