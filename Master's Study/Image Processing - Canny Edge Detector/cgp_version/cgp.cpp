...
/*===============================================================================
  ...
  Nejake upravy od Petra Dvoracka. Changes marked as **modified**
  Puvodni verze totiz byla vhodna pro obvody..
  =============================================================================*/
...
tparams params = {50 /*generations*/, 4 /*pop.size*/, 15 /*mut.genes*/, 0, 0, 50 /*cols*/, 5 /*rows*/, 50 /*lback*/, 2, 1, 16 /*functions*/};
...
inline void cgp_mutate(chromosome p_chrom) 
{
    ...
    for (int j = 0; j < genes; j++) 
    {
        ...
        {  //mutate a primary output
           do { rnd = rand() % params.rows + 52; } while (rnd == p_chrom[i]);
           p_chrom[i] = rnd;
        }
    }
}
    ...
    for (i=0; i < params.cols; i++)
        for (j=0; j < params.rows; j++) 
        {
            ...
            switch (fce) 
            {
              ...
              */
              // NULARNI OPERACE
              case 0: *pnodeout++ = 255; break;          //constanta
              // UNARNI OPERACE
              case 1: *pnodeout++ = in1; break;          //identita
              case 2: *pnodeout++ = ~in1; break;         //not
              case 3: *pnodeout++ = in1 >> 1; break;    //xor
              case 4: *pnodeout++ = in1 >> 2; break;    //xor
              // BINARNI OPERACE
              case 5: *pnodeout++ = in1 | in2; break;    //or
              case 6: *pnodeout++ = ~(in1 | in2); break;    //skoro nor
              case 7: *pnodeout++ = in1 & in2; break;    //and
              case 8: *pnodeout++ = ~(in1 & in2); break;    //nand
              case 9: *pnodeout++ = in1 ^ in2; break;    //xor
              case 10: *pnodeout++ = (in1 << 4) | (in2 << 4); break;    //what the f*ck
              case 11: *pnodeout++ = (in1 + in2) & 255; break;    //plus
              case 12: tmp = (in1 + in2); 
                       *pnodeout++ = (tmp > 255) ? 255 : tmp; 
                       break;    //saturovany plus
              case 13: *pnodeout++ = (in1 + in2) >> 1; break;
              case 14: *pnodeout++ = (in1 > in2) ? in1 : in2; break;
              case 15: *pnodeout++ = (in1 < in2) ? in1 : in2; break; // zombie
              default: abort();
            }
        }
}
...
if (usednodes[i] < 10)
    ...
    for (int l=0; l < params.trainingvectors; l++) 
    {
        ...
        {
            if (i == parentidx) continue;
            if (fitvalues[i] == 0x7fffffff) continue;
            ...
            vysl = (nodeoutput[*p_chrom++] - *(ptraindata));
            fitvalues[i] += ((vysl > 0) ? vysl : -vysl);
        }
...
inline void write(int parentidx, char * fin) 
{ 
      cv::Mat tmp = cv::imread( fin );
      cv::Mat src;
          cv::cvtColor( tmp, src, CV_BGR2GRAY );
    ...
    for (int l=0; l < params.trainingvectors; l++) 
    {
        ...
        fit = 0;
        p_chrom = (chromosome) population[i] + params.geneoutidx;
vysl = nodeoutput[*p_chrom++];
                //    printf("%2x ", vysl);
    
        dst.at<unsigned char>(row_idx, col_idx)=(unsigned char) vysl;
    ...
    printf("\n Used: %d \n", usednodes[i]);
    cv::imwrite( ("cgp_vysledek.png"), dst ); 
    ...
    for (int i=params.cols; i > 0; i--) 
    {
        for (int j=params.rows; j > 0; j--,idx--) 
        {
            ...
            {  
               ...
               in = *pchrom--; // in2
               if (fce > 4)    // 2-input functions
                  isused[in] = 1;
               in = *pchrom--; // in1
               if (fce != 0)    // 2-input functions
                  isused[in] = 1;
...
int main(int argc, char* argv[])
{
    ...
    ///load training data
    if ((data_items = parsefile(argv[1], argv[2], NULL, NULL, NULL)) < 1) 
    { 
       printf("Invalid data\n"); return 0; 
    }
    data = new int[data_items];
    parsefile(argv[1],argv[2], data, &params.inputs, &params.outputs);
    ...
    for (int i=0; i < params.popsize; i++) 
    {
        ...
        for (int j=0; j < params.outputs; j++) // primary outputs
            *p_chrom++ = rand() % params.rows + 58-params.rows;
    }
