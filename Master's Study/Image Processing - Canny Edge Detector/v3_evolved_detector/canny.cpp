 ...
 * Deep balck magic
 */
int evolved_filter(int s0, int s1, int s2, int s3, int s4, int s5, int s6, int s7, int s8){
    int s10 = f6(s3, s0);
    int s11 = f12(s6, s4);
    int s12 = f12(s1, s5);
    int s13 = f5(s11, s12);
...
void canny(int ** img, int ** result, double sigma, int low_thold, int high_thold){
    ...
    magnitude[0] = (int *) malloc(img_width * sizeof(int));
    for(int y = 0; y < img_width; y++)
        magnitude[0][y] = 0;
    for(int x = 1; x < img_height-1; x++){ // Vypocet magnitudy 
        magnitude[x] = (int *) malloc(img_width * sizeof(int));
        magnitude[x][0] = 0;
        for(int y = 1; y < img_width-1; y++){
            magnitude[x][y] = evolved_filter(img[x-1][y-1],img[x  ][y-1],img[x+1][y-1],img[x-1][y],img[x  ][y],img[x+1][y],img[x-1][y+1],img[x  ][y+1],img[x+1][y+1]);
        }
        magnitude[x][img_width-1] = 0;
    }
    magnitude[img_height - 1] = (int *) malloc(img_width * sizeof(int));
    for(int y = 0; y < img_width; y++)
        magnitude[img_height - 1][y] = 0;

    printf("Hello\n");
    save_img(magnitude, "out.png");
}
