...
get_sublist = lambda index, nested_list : map(lambda sublist: sublist[index], nested_list)
...
class CGP(object):
    ...
    def __set_connections(self):
        ...
        self._valid_connection = [(self.rows * (column - self.level_back), self.rows * column) for column in xrange(self.columns)] 
        # Filter invalid indexes which are negative -> make them 0
        self._valid_connection = map(lambda sublist: map(lambda x: x if (x >= 0) else 0, sublist), self._valid_connection)
    ...
    def set_data(self, input_data, training_data):
        ...
        Parameters
            ...
            training_data : list
                The target circuit.
        """
        self.input_data_range = range(-len(input_data), 0)
    ...
    def __create_chromosome(self):
        ...
        connect = lambda connection: random.choice(self.input_data_range + range(connection[0], connection[1]))
        
        # Nodes may differ by their connection between columns. The interconnection is depended on the value of level back. 
        for column_index in xrange(self.columns):
            ...
            create_node = lambda: [connect(valid_con) for _ in xrange(self.maximal_arity)] + [random.choice(self.operations)]
            chromosome += [create_node() for _ in xrange(self.rows)]
    ...
    def __init_pop(self, pop_size):
        ...
        self.pop = [self.__create_chromosome() for _ in xrange(pop_size)]
    ...
    def mutate(self):
        ...
        for _ in xrange(random.randint(1, self.mutation_rate+1)): # Mutate it at least once. Else the chromsome wouldn't change.
    ...
    def init_eval(self):
        ...
        buffer_data = deepcopy(self.input_data) + range(self.rows * self.columns + len(self.training_data))
        buffer_indexes = range(-len(self.input_data), len(buffer_data) - len(self.input_data))
    ...
    def chrom_to_str(self, chromosome):
        ...
        max_length_of_operation_string = max(map(len, self.operations_names))
        chrom_str = ""
        for i in xrange(self.rows):
            ...
            for j in xrange(self.columns):
                ...
                aligned_inputs = map(lambda x: str(x).rjust(max_input_strlen), inputs)
    ...
    def run(self, pop_size, mutations, generations):
        ...
        # Run evolution
        for self.generation in xrange(generations):
            # Evaluate pop
            map(self.eval, self.pop)
